...
In order to complete this workshop, you will need the following tools installed on your laptop:
...
You can also use a VirtualBox with [Ubuntu](https://www.osboxes.org/ubuntu/) for example, to install the tools.
...
2. Fill in the registration form and follow the link to confirm your account in the **confirmation email** once it arrives.
...
The lite account provides free access to a subset of IBM Cloud resources. Lite accounts **do not need a credit-card** to register and they **do not expire** after a certain period of time. 
...
In order to execute our workshop easily, we provide **promotional codes** to create free Kubernetes clusters, so no credit card details are required.
...
_Note:_ The creation of a cluster can take up to **20 minutes**.
...
4. Copy and paste the **output** from the command of the previous step to set the `KUBECONFIG` environment variable and configure the CLI to run `kubectl` commands against the cluster:
...
8. Install Istio:
    ...
    ```
    e. Then open http://localhost:20001/kiali in your browser, log in with the following credentials
...
If you get the following output in the ```./iks-scripts/create-registry.log``` logfile ```The requested namespace is already in use```.
...
Change the registry name in the ```local.env``` file to ```REGISTRY_NAMESPACE=cloud-native-YOUR_NAME``` , for example.
...
_Note:_ When you're using the CLI or the Cloud console in a browser, always make sure you're **viewing the correct region**, as your resources will only be visible in the region it was created in.

---

### 3.7 Accessing the Kubernetes cluster manually (optional) <a name="part-SETUP-05"></a>

Now let's see how to set the context using the ```kubectl``` CLI, how to access the Kubernetes dashboard, and how to gather basic information about our clusters.
